...
exports.getRecordsInfoByName = (name)=>{
  ...
  where S.sname="曹昊" and R.sid = S.sid and R.cid = C.cid;`
  return query(_sql)
}
//right
exports.getRecordsInfoBySid = (sid) =>{
  let _sql = `select C.*
  from Records AS R, Courses C
  where Se.sid = "${sid}" and C.cid = R.cid;`
  return query(_sql)
}
...
exports.insertCourse = (value)=>{
  let _sql = `insert into Courses set course_id=?,cname=?,tname=?,redits=?,grade=?,cancle_year=?`
  return query(_sql,value)
}
...
exports.insertRecord = (value)=>{
  let _sql = `insert into Records set sid=?,cid=?,select_year=?,grade=?;`
  return query(_sql,value)
}
...
exports.updateRecord = (body)=>{
  //在这里拼接字符串
  let _sql = `update Records set grade = "${body.grade}" where cid = "${body.cid}" and sid = "${body.sid}";`
  return query(_sql)
}
