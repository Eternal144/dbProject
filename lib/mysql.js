...
exports.getBaseInfoBySid = (sid)=>{
  let _sql = `select * from Students where sid = "${name}";`
  return query(_sql)
}

//right
exports.getSelectInfoByName = (name)=>{
  let _sql = `select Se.* 
  from Selective AS Se, Students AS S, Courses AS C
  where S.sname="${name}" and Se.sid = S.sid;`
  return query(_sql)
}
//right
exports.getSelectInfoBySid = (sid) =>{
  let _sql = `select Se.*
  from Selective AS se
  where Se.sid = "${sid}";`
  return query(_sql)
}
